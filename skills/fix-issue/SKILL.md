---
name: fix-issue
description: "Automate the full GitHub issue bug-fix pipeline end-to-end. Use when the user says 'fix issue', 'fix #123', 'fix this bug', 'resolve issue #XXX', 'fix and ship this issue', or passes a GitHub issue number for autonomous bug resolution. Reads the GitHub issue, implements the fix, reviews it, commits, creates a PR, links the issue, and comments with a summary. Also use when the user wants to automate the fix-review-commit-PR cycle for any GitHub issue."
---

# Fix Issue

Automate the full lifecycle of a GitHub issue: read the issue, implement the fix, review changes, commit, push, create a PR that closes the issue, and comment with a summary. Zero-touch pipeline from issue to PR.

## Prerequisites

**Read `CONFIG.md` at the start of every run.** This file contains project-specific settings (GitHub repo, team members, labels, deployment config, QA credentials). If `CONFIG.md` doesn't exist, warn the user and point them to `CONFIG.template.md`.

## When to Use

- User passes a GitHub issue number (e.g., `#123` or `123`) and wants it fixed end-to-end
- User says "fix this issue", "resolve this bug", "fix and ship #XXX"
- Any bug issue that needs the full fix-review-commit-PR cycle
...
| `repo` | (current repo) | Target repo in `owner/repo` format. Defaults to the repo detected from `git remote get-url origin`. |
| `skip-review` | `false` | Skip the review-team phase |
| `skip-github` | `false` | Skip GitHub PR creation, issue comment, and label updates (just fix and commit) |
| `assignee` | (from CONFIG.md) | GitHub username to assign the PR to |
...
/fix-issue 123 branch=new                 # create fix/issue-123 branch
...
/fix-issue 123 repo=myorg/myrepo          # explicit repo
```
...
|     2. Separate branch — create fix/issue-{N} branch (for PR workflow)
...
|   If branch=new: create fix/issue-{N} branch
|   If branch=main: stay on current branch
|
+-- Phase 1: READ ISSUE
|   GitHub CLI -> issue details, comments, labels, linked PRs
|
...
|   Reproduce the bug steps from the issue in headless Chrome
...
|   Create PR that closes the issue (or link commit if branch=main)
|   Add label to issue: "in-review" or "fixed" (from CONFIG.md)
|   Post comment on issue: what was fixed, PR link, QA steps, review findings
|   If issue has existing comments: respond contextually
|
+-- Phase 9: SUMMARY
|   Report to user: what was done, commit hash, PR link, deployment status
```
...
Phase 8:   Was `skip-github=true` in the user's invocation? YES -> skip. NO -> run it.
```
...
- **Phase 7 (Deploy Monitor)**: A broken build = testers can't verify.
...
- [ ] **GitHub updated**: PR created (or commit linked), issue labeled and commented (or `skip-github=true`)
...
**First: Read `CONFIG.md`** to load project-specific settings (team members, labels, deployment config, QA credentials). If not found, warn user and point to `CONFIG.template.md`.
...
2. **Separate branch** — create a `fix/issue-{N}` branch (for PR workflow)
3. **Worktree** — isolated worktree (for running parallel fix-issues)
```
...
| **Separate branch** | `git checkout -b fix/issue-{N}` — create the branch now so all work happens on it. |
| **Worktree** | Call `EnterWorktree` with `name` = `fix-issue-{N}`. Then immediately symlink `.env.local` and `node_modules` from the main repo root. All subsequent phases run inside the worktree. |

Store the resolved branch strategy — Phase 6 and Phase 6.5 need it for commit/push/merge logic.

### Phase 1: Read Issue

Fetch the GitHub issue using the `gh` CLI:

```bash
gh issue view {issueNumber} --repo {owner/repo} --json number,title,body,labels,comments,assignees,milestone,state
```

Also fetch any linked PRs:

```bash
gh issue view {issueNumber} --repo {owner/repo} --json closedByPullRequests,linkedBranches
```

Extract:
- **Title**: What the bug is
- **Body**: Full context, repro steps, expected vs actual behavior
- **Labels**: Priority/type signals
- **Comments**: Any discussion, additional context, or previous fix attempts
- **Linked PRs**: Check if a fix is already in progress

If the `gh` fetch fails (e.g., private repo, auth issue), ask the user to describe the bug manually and continue.

**Detect repo** if not passed explicitly:
...
4. **Reproduce the bug** — follow the reproduction steps from the issue body. Screenshot each step:
   ```bash
   playwright-cli screenshot --filename=playwright-qa-screenshots/issue-{N}-verify/NN-step.png
   ```
...
- Medium/low issues: Note them in the GitHub comment, fix if quick
...
3. **Commit with known issue** — Ship what we have and note the remaining issue in a GitHub comment.
...
| **separate branch** | Already on `fix/issue-{N}` from Phase 0 — just commit |
...
The `Closes #N` trailer automatically closes the GitHub issue when the commit lands on the default branch (or when the PR merges).
...
#### PR Creation (for separate branch and worktree strategies)
...
gh pr create \
  --repo {owner/repo} \
  --title "fix: <concise description> (#<issue-number>)" \
  --body "$(cat <<'EOF'
## Summary

<1-3 bullet points describing what was fixed>
...
<table or bullet list of files changed and what was done>
...
<summary of review findings or "No issues found">
...
If `branch=main`, skip PR creation — the commit already closes the issue via the `Closes #N` trailer.

### Phase 7: Deployment Monitor

**Skip if `skip-deploy=true`.**

**After pushing, monitor the deployment to confirm the build succeeds before handing off.**
...
Execute these GitHub operations in parallel:

**1. Apply label to the issue** (label names from CONFIG.md):

```bash
gh issue edit {issueNumber} --repo {owner/repo} --add-label "in-review"
```

| Condition | Label |
|-----------|-------|
| Review had unresolved medium+ findings | `in-review` |
| Normal fix (review clean or all findings resolved) | `fixed` |
| Trivial fix + `skip-review=true` + build passes | `fixed` |
...
**Deployment**: {Ready at <url> / Monitoring skipped}

### What was fixed

{root cause and solution in 1-2 sentences}
...
{summary or "No issues found" or "Skipped"}

---
*Fixed by Claude Code — [fix-issue skill](https://github.com/EricTechPro/startup-claude-skills)*
EOF
)"
```

If the issue has existing comments with questions or context, acknowledge them in the comment.

**3. Request reviewers on the PR** (if a PR was created):

```bash
gh pr edit {prNumber} --repo {owner/repo} --add-reviewer {reviewers-from-config}
```
...
| 8. GitHub Handoff | {Done — PR + comment / Skipped} |
| 9. Summary | Done |
```
...
| Scenario | Action |
|----------|--------|
| `gh` CLI fails to read issue | Ask user to describe the bug, continue without issue context |
| `gh` auth error | Run `gh auth login`, retry |
...
| PR creation fails | Log warning, share the push URL, tell user to create PR manually |
...
| Label not found on repo | Create label via `gh label create`, then apply |
...
3. **Worktrees for parallelism** — Use `branch=worktree` to run multiple `/fix-issue` in separate terminals simultaneously
4. **Existing comments matter** — The skill reads issue comments and responds contextually
5. **`Closes #N` in commit message** — When pushing directly to main, this automatically closes the issue
...
| 8 | **GitHub handoff** | PR created + issue labeled + issue commented (unless `skip-github=true`) | Any GitHub op missing |
...
| 17 | **PR body quality** | PR body includes summary, root cause, files changed, QA steps, and `Closes #N` | Missing sections or no issue link |
| 18 | **Repo detection** | `owner/repo` correctly inferred from `git remote get-url origin` when not passed | Hardcoded or wrong repo |
